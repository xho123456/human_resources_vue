    ...
    <div class="body_2">
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <el-drawer v-model="drawer" title="I am the title" :with-header="false">
          <h3 style="margin-top:1px;">定薪1</h3><br/>
        ...
        <el-tab-pane label="固定工资" name="first">
          ...
          <el-table :data="tableData" style="width: 100%">
            ...
            <el-table-column prop="fixedwageOfficialmoney" label="正式工资" width="120" />
        ...
        <el-tab-pane  name="second" >
          ...
          <el-select v-model="pageInfo1.deptName" placeholder="请选择部门" style="position: absolute;right: 500px; width: 150px;" >
            <el-option
                v-for="item in options"
                :key="item.value"
                :label="item.deptName"
          ...
          <el-table :data="tableData1" style="width: 100%">
            ...
            <el-table-column prop="afterSalary" label="调薪后基本工资" width="120" />
            <el-table-column prop="salaryState" label="状态" width="120" >
            <template #default="scope">
              <span v-if="scope.row.salaryState===1">同意</span>
              <span v-if="scope.row.salaryState===0">未审批</span>
              <span v-if="scope.row.salaryState===2">不同意</span>
            </template>
            </el-table-column>
            <el-table-column prop="operator" label="操作人" width="120" />
            <el-table-column prop="salaryRemarks" label="备注" width="120" />
            <el-table-column prop="takeEffectDate" label="生效日期" width="120" />
            <el-table-column prop="createdTime" label="申请日期" width="120" />
...
export default {
  ...
  methods: {
    selectEndAuditflow1() {
      this.axios({
        method:'post',
        url:"http://localhost:8007/provider/Salary/Salary",
        data:this.pageInfo1,
        responseType:'json',
    ...
    addSalary(){
      this.axios({
        method:"post",
        url:"http://localhost:8007/provider/adSalary/salary",
        data:this.Salary,
        responseType:'json',
        responseEncoding:"utf-8",
      }).then(response=>{
    ...
    updatedept(){
      this.axios({
        method:"put",
        url:"http://localhost:8007/provider/updateFixedSalary/fixedSalary",
        data:this.fixedSalary,
        responseType:'json',
        responseEncoding:"utf-8"
      }).then(response=>{
        console.log(response)
    ...
    selectDept() {
      this.axios
          .get("http://localhost:8007/provider/FixedSalarySelectDept", {
            params:this.options,
          })
          .then((response) => {
            console.log("查询cao");
    ...
    selectEndAuditflow() {
      this.axios({
        method:'post',
        url:"http://localhost:8007/provider/FixedSalarySelect/DeptName",
        data:this.pageInfo,
        responseType:'json',
        responseEncoding:'utf-8'
      })  .then((response) => {
